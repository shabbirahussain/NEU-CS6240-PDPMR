...
/**
  ...
  */
class KMeans(MostPopMax: Int, NumIter:Int, outputPath:String)
  ...
  override def exec(sc: SparkContext, songs: RDD[SongRecord], artist: RDD[ArtistRecord], similar: RDD[SimilarArtist]) {
    ...
    super.saveCentroids(newCentroids, ignoreScores = false, 0)
    for (i <- 1 to NumIter) {
      System.out.println("\nIteration " + i)
      ...
      val clstrAssgn = centXArt                                 //(term, (artist_id, (cluster_id, score)))
        ...
        .reduceByKey((u, v) => if (u._2 > v._2) u else v)       //(artist_id, max{(cluster_id, sum_score)})
      ...
      val cnt = d                                               //((artist_id, cluster_id), (term, score))
        .map(x => x._1.swap)                                    //(cluster_id, artist_id)
        .distinct                                               //-- remove repeated terms
        .combineByKey[Int](                                     //(cluster_id, cnt_artist_id)
          (v: String) => 1,
          (u: Int, v: String) => u + 1,
          (u1: Int, u2: Int) => u1 + u2)

      newCentroids = d.map(x => ((x._1._2, x._2._1), x._2._2))  //((cluster_id, term), score)
        .reduceByKey(_ + _)                                     //((cluster_id, term), sum_score)
        .map(x => (x._1._1, (x._1._2, x._2)))                   //(cluster_id, (term, sum_score))
        .join(cnt)                                              //(cluster_id, ((term, sum_score), cnt_artist_id))
        .map(x => (x._2._1._1, (x._1, x._2._1._2 / x._2._2)))   //(term, (cluster_id, sum_score / cnt_artist_id))
      ...
      // Save centroids
      super.saveCentroids(newCentroids, ignoreScores = false, i)
    }
  }
}
