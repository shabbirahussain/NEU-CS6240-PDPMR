...
/**
  ...
  */
case class SongRecord(line: String) extends Serializable {
  ...
  private def toSafeDouble(v: String, default: Double): Double = try {
    v.toDouble
  } catch {
    case _: Exception => default
  }

  private def toSafeInt(v: String, default: Int): Int = try {
    v.toDouble.toInt
  } catch {
    case _: Exception => default
  }
